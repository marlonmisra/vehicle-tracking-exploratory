...
def process():	
	...
	for non_vehicle_name in non_vehicle_names:
		non_vehicle_image = mpimg.imread(non_vehicle_name)
		images.append(non_vehicle_image)
		y.append(0)
	print("Finished reading non-car data")

	#get vehicle images and labels
	vehicle_names_far = glob.glob('./vehicles/GTI_Far/image*.png')
	for vehicle_name in vehicle_names_far:
		vehicle_image = mpimg.imread(vehicle_name)
		images.append(vehicle_image)
		y.append(1)

	vehicle_names_left = glob.glob('./vehicles/GTI_Left/image*.png')
	for vehicle_name in vehicle_names_left:
		vehicle_image = mpimg.imread(vehicle_name)
		images.append(vehicle_image)
		y.append(1)

	vehicle_names_middle = glob.glob('./vehicles/GTI_MiddleClose/image*.png')
	for vehicle_name in vehicle_names_middle:
		vehicle_image = mpimg.imread(vehicle_name)
		images.append(vehicle_image)
		y.append(1)

	vehicle_names_right = glob.glob('./vehicles/GTI_Right/image*.png')
	...
	pickle.dump(X_scaler, open("normalizer.sav", 'wb'))
